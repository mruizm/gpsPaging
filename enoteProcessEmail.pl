...
for (;;)
{
    ...
    for (; $_ = <INFILE>; $last_byte = tell)
    {
        ...
        #Subject: CIBC : N-IM009801154 (Priority 2) - TTO 50 Percent
        ...
        if ($line =~ m/(^N-IM[\d|-]+)\shas been\sdispatched\sto\sassignment\sgroup\s?([\w\d|-]+)/)
        {
        	...
        	$dispatched_ticket_workgroup = $2;
       		if (($dispatched_ticket_id eq $dispatched_ticket_id_body) || ($dispatched_ticket_workgroup ne "W-INCFLS-ESM-RBA"))
        	{
