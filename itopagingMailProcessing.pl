...
# Version 3.0                   Initial 3.0 Release
# Version 3.1                   Fixed the issue when workgroup is displayed in two lines (CIBC WINDOWS/UNXI WG)
# Version 3.2                   Fixed issues with whitespaces within ticket's workgroup
# Version 3.3                   Removes ' character from subject to permit SMS delivery
# Version 3.4                   Adds capability to decode emails in base64 codification
# # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # # #
...
our $dbh = DBI->connect('dbi:mysql:dbname=itopagingdb_staging;port=3306;host=gccovo01.cri.hp.com','gpspaging','GPSroot2013',{AutoCommit=>0,RaiseError=>0,PrintError=>1});

#Init of variables
my $ENOTE_MAIL_BYTES_FILE = '/opt/itopager/tmp/enote_mail_bytes_staging.tmp';
my $loggerDBfile = "/opt/itopager/log/pagingDBerror_staging.log";
my ($last_byte, $line, $serverDate, $alert_page);
my ($sth, $sth_insert, $attn_dtv_msj);
my ($dispatched_ticket_id_body, $ticket_customer, $full_ticket_subject, $dispatched_ticket_id, $ticket_dispatched_prio, $sms_subject, $sms_ticket_subject);
...
my @results;
my ($wgLine, $wgLineComplete2, $base64Line, $decoded) = "";
my $addedInDb = 0;
my $countWhite = 0;
my $IswgLineComplete = 0;
my $wgLineComplete1 = "";
...
my $FILE = '/var/spool/mail/itopager';
...
for (;;)
{
    ...
    for (; $_ = <INFILE>; $last_byte = tell)
    {
        ...
        if ($line =~ m/^Subject:\s(([\w|\-|\s|\d]+\s):\s(N-IM[\d|-]+)\s(\(.+\))\s-\sDispatched)/)
        {
            ...
            $full_ticket_subject =~ s/^\s+|\s+$//g;
            chomp($ticket_customer);
            $ticket_customer =~ s/^\s+|\s+$//g;
            chomp($dispatched_ticket_id);
            $dispatched_ticket_id =~ s/^\s+|\s+$//g;
            chomp($ticket_dispatched_prio);
            $ticket_dispatched_prio =~ s/[\(|\)]//g;
            $ticket_dispatched_prio =~ s/^\s+|\s+$//g;
        }
        if (/(^N-IM[\d|-]+)\shas been\sdispatched\sto\sassignment\sgroup\s?([\w\d|-]+)=$/ .. /:/)
        {
            $wgLine = $_;
            chomp($wgLine);
            $wgLineComplete1 = $wgLineComplete1.$wgLine;
        }
        if ($wgLineComplete1 =~ m/:/)
        {
            $wgLineComplete1 =~ m/(.*)(Task Title|Service:)/;
            $wgLineComplete1 = $1;
            chomp($wgLineComplete1);
            $wgLineComplete1 =~ s/=//g;
            $IswgLineComplete = "1";
        }
        if ($IswgLineComplete eq "1")
        {
            if ($wgLineComplete1 =~ m/(.*)\shas been dispatched to assignment group\s(.*)/)
            {
                ...
                $sth = $dbh->prepare("SELECT ticket_id FROM ticket_in_dispatched
                                        WHERE ticket_id = ? AND ticket_workgroup = ?");
                $sth->execute($dispatched_ticket_id, $dispatched_ticket_workgroup)
                ...
                #If ticket_id is found within ticket_in_dispatched table
                if ($if_ticket_exists_in_db > 0)
                {
                    $addedInDb = 1;
                }
                #Add ticket_id entry within ticket_in_dispatched table
                else
                {
            ...
            $if_ticket_exists_in_db = 0;
                ...
                ###### BASE 64 processing #####
                #if (/Content-Transfer-Encoding: base64/ .. /(--.*|^From)/)
                #{
                #       $base64Line = $base64Line.$line;
                #       if ($line =~ m/^\s*$/)
                #       {
                ...
                #{
                #       #chomp($base64Line);
                #       $base64Line =~ s/Content-Transfer-Encoding: base64(MIME-Version: 1.0)?//;
                #       $base64Line =~ s/^--_.*_//;
                #       $base64Line =~ s/^From.*\d\d\d\d//;
                #       #print "$base64Line\n";
                #       $decoded = decode_base64($base64Line);
                #       chomp($decoded);
                #       if ($decoded =~ m/(^N-IM[\d|-]+)\shas been\sdispatched\sto\sassignment\sgroup\s?([\w\d|-]+)(.*)/)
                #       {
                #               $dispatched_ticket_id_body = $1;
                #               #print "64 64 $decoded\n";
                #               $dispatched_ticket_workgroup = $2;
                #               chomp($dispatched_ticket_id_body);
                #               $dispatched_ticket_id_body =~ s/^\s+|\s+$//g;
        #
                #               chomp($dispatched_ticket_workgroup);
                #               $dispatched_ticket_workgroup =~ s/[\(|\)]//g;
                #               $dispatched_ticket_workgroup =~ s/^\s+|\s+$//g;
                #               chomp($dispatched_ticket_workgroup);
                #
                #       }
                #       print "Running 64 decoded line routine\n";
        #   $sms_subject = "ALERT: $ticket_dispatched_prio - $dispatched_ticket_id";
        #    chomp($sms_subject);
        #    print "$sms_subject - $dispatched_ticket_workgroup\n";
        ...
        #       if (($dispatched_ticket_id eq $dispatched_ticket_id_body) && ($dispatched_ticket_workgroup ne "W-INCFLS-ESM-RBA") && ($if_ticket_exists_in_db eq "0"))
        #       {
        #               print "Adding to DB 64 decode routine\n";
        #               $sth = $dbh->prepare("INSERT INTO ticket_in_dispatched
        #                            (ticket_id, ticket_subject, ticket_customer, ticket_priority, ticket_workgroup, ticket_date_added_db, sms_message_to_mobile, ticket_processed, ticket_date_processed)
        #                            values
        #                            (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        if ($line =~ m/^Subject:\s(([\w|\-|\s|\d]+\s):\s(N-IM[\d|-]+)\s(\(.+\))\s-\s(TT[R|O]\s[\w|\d|-|\s]+))/)
        {
            ...
            $ttr_percent = $5;
            chomp($ttr_full_desc);
            $ttr_customer =~ s/^\s+|\s+$//g;
            $ttr_percent =~ s/Group//gi;
            chomp($ttr_customer);
            chomp($ttr_ticket_id);
            ...
            $ttr_ticket_prio =~ s/[\(|\)]//g;
            chomp($ttr_ticket_prio);
            $sms_ttr_subject = "ALERT: $ttr_ticket_prio - $ttr_ticket_id - $ttr_percent_raw";
            chomp($sms_ttr_subject);
            print "$sms_ttr_subject\n";
            $sth = $dbh->prepare("SELECT ticket_workgroup FROM ticket_in_dispatched
                                  WHERE ticket_id = ? ORDER BY ticket_date_added_db DESC LIMIT 1");
...
sub processEntriesToNotificationTable{
        ...
        my $processEntriesLogfile = "/opt/itopager/log/processingSmserror_staging.log";
...
sub queryGetUsernamesUsingworkgroup{
        ...
        my @userNamesArray = ();
        my $userName;
        my $processEntriesLogfile = "/opt/itopager/log/pagingDBerror_staging.log";
        my $serverDate = DateTime->now(time_zone => 'America/Costa_Rica');
...
sub queryUserNamePrioritySubscription{
        ...
        chomp($priorityTicketCode);
        my $priorityTicketPercent = $_[3];
        chomp($priorityTicketPercent);
